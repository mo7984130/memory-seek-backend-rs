...
use types::media::MergePersonParam;
use types::media::face::FaceRecord;
use types::media::person::{PersonId, PersonRecord, UpdatePersonRecord};
...
use crate::mappers::media_mapper::MediaMapper;
...
// 查询
impl PersonRepo {
    pub async fn query_all(state: &MediaState) -> Result<Vec<PersonRecord>> {
    ...
    pub async fn search_person_page(
        ...
        size: u64,
    ) -> Result<CursorPage<types::media::person::PersonRecord, ()>> {
        PersonMapper::query_search(&state.db, keyword, cursor, size)
            .timed(metrics_name!("query_search"))
            .await
    }

    pub async fn load_faces_with_media_files(
        state: &MediaState,
    ) -> Result<(Vec<FaceRecord>, HashMap<MediaId, String>)> {
        let faces = FaceMapper::query_all(&state.db)
            .timed(metrics_name!("query_faces"))
            .await?;
        let ids = faces
            .iter()
            .map(|face| face.media_id)
            .collect::<HashSet<_>>();

        let files = MediaMapper::query_id_and_file_id_by_ids(&state.db, &ids)
            .timed(metrics_name!("query_media_files"))
            .await?;
        Ok((faces, files))
    }
}
