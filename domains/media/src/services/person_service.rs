...
use types::{
    ...
    media::{
        MergePersonParam, PersonCursorParam, PersonMediaCursorParam, PersonSearchParam, PersonView,
        MediaView, RenamePersonParam, SecondaryClusterParam,
        face::FaceRecord,
...
impl PersonService {
    ...
    pub async fn full_scan(state: Arc<MediaState>, admin: AdminId) -> Result<()> {
    ...
    /// 执行全量扫描, 重建人物聚类和人脸归属.
    pub async fn inner_full_scan(state: Arc<MediaState>, admin: AdminId) -> Result<()> {
        let user_id = admin.into_inner();
        info!(user_id = %user_id, "管理员触发人物全量聚类");

        info!("加载照片数据");
        let (faces, media_file_ids) = PersonRepo::load_faces_with_media_files(&state).await?;
            ...
            for cluster in cluster_result.0 {
                ...
                let person = NewPerson {
                    ...
                    cover: PersonCover {
                        ...
                        media_id: cover_face.media_id,
                        face_score: cover_face.score,
                        file_id: media_file_ids
                            .get(&cover_face.media_id)
                            .cloned()
                            .ok_or_error(
                                "person_cover_media_not_found",
                                "封面人脸所属照片不存在",
                                AppError::InternalServerError,
                            )?,
...
impl PersonService {
    ...
    pub async fn merge_person(
        ...
        PersonView::from_record(
            person,
            admin.into_inner(),
            MediaRepo::get_media_dimensions(state, file_id).await?,
        )
        .to_ok()
    }
}
...
impl PersonService {
    ...
    /// 获取人物的照片列表(游标分页, 参照 `CollectionMediaService::get_medias`)
    #[common_macros::metered]
    #[tracing::instrument(
        skip_all,
    ...
    /// 查询人物关联的照片并生成照片视图.
    pub async fn get_person_medias(
        state: &MediaState,
        user_id: UserId,
        person_id: PersonId,
        req: PersonMediaCursorParam,
    ) -> Result<CursorPage<MediaView, TimeIdCursor<MediaId>>> {
        let page = FaceRepo::query_person_media_ids(state, person_id, &req)
