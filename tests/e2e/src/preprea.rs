...
const SEED_STATEMENTS: [&str; 14] = [
    ...
    // 7. 照片元数据(每个 photo 用户预置若干张; file_id 唯一, 无需真实对象存储)
    ...
    // 8. 时间线统计(当前月份, 供 /photo/timeline/stats)
    "
    ...
    // 9. 人脸(每张 seed 照片 1 张, 初始未分配; embedding 为随机 512 维)
    "
    ...
    // 10. 人物(每 photo 用户 1 个, cover 用其第一张照片; centroid 随机 512 维)
    "
    ...
    SELECT u,
           ...
           now()
    FROM generate_series(1, :PHOTO_USERS) AS u",
    // 11. 每个用户前 FACES_PER_PERSON 张照片的人脸归属到对应人物
