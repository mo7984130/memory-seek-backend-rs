...
use types::{
    ...
    media::{
        FaceView,
        dto::face::{FaceDeleteBatchResult, UnassignedFaceMediaCursorParam},
        dto::media::MediaView,
...
use crate::{
    ...
    repo::FaceRepo,
    services::media_service::{AfterMediaUpload, MediaService},
};
...
impl FaceService {
    /// 人脸计算.
    /// 全量时, 备份并且清空表, 从头开始
    /// 增量时, 备份表, 靠是否有media无face来判断
    pub async fn compute(state: Arc<MediaState>, admin: AdminId, full: bool) -> Result<()> {
    ...
    async fn compute_inner(state: Arc<MediaState>, admin: AdminId, full: bool) -> Result<()> {
        let user_id = admin.into_inner();
        ...
        let batch_size = 128;
        let mut previous_id = MediaId(0);
        let mut batch_idx = 0i64;
        let mut total_medias = 0u64;
        ...
        loop {
            ...
            let medias = FaceRepo::query_face_compute_medias(&state, full, batch_size, previous_id)
            ...
            // 刷新previous_id
            if let Some(last) = medias.last() {
                previous_id = last.0;
            }
            let media_count = medias.len();

            let mut new_faces: Vec<face::NewFaceRecord> = Vec::with_capacity(media_count * 4);
            let _download_batch_timer = MetricsTimer::start(metrics_name!("download_batch"));
            for (media_id, file_id) in medias {
                debug!("照片流程开始: media_id: {media_id}");
                let _ = async {
                    let img = Self::download_media(&state, &file_id)
                        .timed(metrics_name!("media_download"))
                        .await?;
                    let faces = Self::detect_media(&state, img)
                        .timed(metrics_name!("media_detect"))
                        .await?;
                    let face_count = faces.len();
                    new_faces.extend(
                        faces
                            .into_iter()
                            .map(|face| face::NewFaceRecord::from_detected(media_id, face)),
                    );
                    Ok::<usize, AppError>(face_count)
                }
                .await
                .inspect(|&face_count| {
                    inc_counter!("medias_processed", 1);
                    inc_counter!("faces_detected", face_count as u64);
                    total_medias += 1;
                    total_faces += face_count as u64;
                    if face_count == 0 {
                        inc_counter!("no_face_medias", 1);
                        total_no_face += 1;
                    }
                })
                .inspect_err(|_| {
                    tracing::warn!(media_id = %media_id, %file_id, "照片流程错误, 跳过");
                });
            }
        ...
        set_gauge!("total_medias", total_medias as f64);
    ...
    /// 从对象存储下载照片并解码为图像缓冲区.
    async fn download_media(state: &MediaState, file_id: &String) -> Result<Img> {
        debug!("下载照片{}", file_id);
        let bytes = state
            .s3_client
        ...
        let img = Self::decode_media(bytes).await?;
    ...
    async fn decode_media(bytes: bytes::Bytes) -> Result<Img> {
        let _decode_timer = MetricsTimer::start(metrics_name!("media_decode"));
    ...
    /// 在阻塞线程中执行人脸检测并返回检测结果.
    async fn detect_media(state: &MediaState, img: Img) -> Result<Vec<Face>> {
        debug!("检测照片中");
    ...
    /// 批量写入检测到的人脸记录.
    async fn insert_faces(state: &MediaState, faces: Vec<face::NewFaceRecord>) -> Result<()> {
        debug!("插入人脸到数据库中");
        if faces.is_empty() {
            debug!("faces为空, 跳过");
        } else {
...
impl FaceService {
    ...
    pub async fn change_face_belonging(
        ...
        // （其当前 span 即本函数，指标为 media:change_face_belonging:db_transaction），此处不再重复计时。
...
impl FaceService {
    /// 查询指定照片的人脸.
    #[common_macros::metered]
    #[tracing::instrument(skip_all, fields(media_id = %media_id))]
    pub async fn get_faces_by_media_id(
        state: &MediaState,
        media_id: MediaId,
    ) -> Result<Vec<FaceView>> {
        let (faces, person_names) =
            FaceRepo::query_faces_with_person_names(state, media_id).await?;
    ...
    /// 分页查询包含未分配人脸的照片.
    pub async fn get_unassigned_face_medias(
        state: &MediaState,
        user_id: UserId,
        req: UnassignedFaceMediaCursorParam,
    ) -> Result<CursorPage<MediaView, TimeIdCursor<MediaId>>> {
        let page = FaceRepo::query_unassigned_face_media_ids(state, &req)
        ...
        let medias = MediaService::load_medias_info(state, user_id, &page.records)
            .timed(metrics_name!("load_medias_info"))
            .await?;

        page.replace_records(medias)
            .with_next_cursor(|media| TimeIdCursor {
                time_at: media.created_at,
                id: media.id,
            })
            .to_ok()
    }
...
impl FaceService {
    ...
    pub async fn delete_face(state: &MediaState, face_id: FaceId, user_id: UserId) -> Result<()> {
...
impl FaceService {
    ...
    async fn on_after_media_upload(
        ...
        let image = Self::decode_media(event.file_data.clone()).await?;
        let faces = Self::detect_media(&state, image)
            .timed(metrics_name!("media_detect"))
            .await?;
        let faces = faces
            .into_iter()
            .map(|face| face::NewFaceRecord::from_detected(event.media.id, face))
            .collect();
...
impl FaceService {
    async fn on_media_delete(
        ...
        let faces = FaceMapper::lock_by_media_ids(txn, &media_ids).await?;
        if faces.is_empty() {
            return Ok(());
        }
        ...
        FaceMapper::delete_by_media_ids(txn, &media_ids).await?;
