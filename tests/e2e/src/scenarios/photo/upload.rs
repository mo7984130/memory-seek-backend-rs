...
#[derive(Default)]
pub struct UploadPhotoScenario;

impl Scenario for UploadPhotoScenario {
    type Ctx = Context;
    ...
    async fn validate(
        ...
        let Some(row) = photo_entity::Entity::find_by_id(view.id)
            .one(&ctx.db)
            .await
            .unwrap()
        else {
...
#[derive(Default)]
pub struct UploadDuplicatePhotoScenario;

impl Scenario for UploadDuplicatePhotoScenario {
    type Ctx = Context;
