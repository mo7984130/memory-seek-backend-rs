...
use crate::media::models::PersonName;
use crate::media::person::{PersonId, PersonRecord};
use crate::media::media::MediaId;
use crate::media::{ImageDimensions, ImageToken, ImageTokenStr};

crate::in_dto!(PersonCursorParam, "media/", serde_default, docs = "人物列表参数(cursor 为 FaceCountIdCursor<PersonId> 的 Base64 编码, 按 face_count 倒序分页)"; {
...
crate::in_dto!(PersonSearchParam, "media/", serde_default, docs = "人物搜索参数(keyword 前缀匹配完整名字或姓名首字母)"; {
...
crate::out_dto!(PersonView, "media/", rename = "Person"; {
    pub id: PersonId,
    pub name: String,
    /// 封面图 token(加密串, 经 `GET /media/image/{token}` 访问)
...
/// 返回人物照片分页的默认页大小.
fn person_media_cursor_page_default_size() -> u64 {
    PERSON_PHOTO_CURSOR_PAGE_DEFAULT_SIZE
}

crate::in_dto!(PersonMediaCursorParam, "media/", docs = "人物照片游标参数(cursor 为 TimeIdCursor<MediaId> 的 Base64 编码)"; {
    #[cfg_attr(feature = "ts", ts(type = "string | null"))]
    pub cursor: Option<TimeIdCursor<MediaId>>,
    #[validate(range(min = 1, max = 1024, message = "分页大小在 1 到 1024 之间"))]
    #[serde(default = "person_media_cursor_page_default_size")]
    #[cfg_attr(feature = "ts", ts(type = "number"))]
    #[cfg_attr(feature = "ts", ts(optional = nullable))]
    pub size: u64,
});

crate::in_dto!(RenamePersonParam, "media/", docs = "重命名人物参数"; {
    #[validate(nested)]
...
crate::in_dto!(SecondaryClusterParam, "media/", docs = "二次聚类参数(将未分配人脸按 centroid 余弦相似度指派到已有人物)"; {
...
#[cfg_attr(feature = "ts", ts(export, export_to = "media/"))]
