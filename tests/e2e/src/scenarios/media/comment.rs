...
use types::media::comment as comment_entity;
...
async fn comment_setup(ctx: &Context, task: &TaskIndex) -> Result<CommentSetup, HttpError> {
    ...
    let media = seed_media(ctx, 6).await.ok_or_else(super::seed_missing)?;
    Ok(CommentSetup {
        session,
        media_id: media.id.0,
    })
...
impl Scenario for PublishCommentScenario {
    ...
    async fn run(
        ...
        publish(ctx, &setup.session, MediaId(setup.media_id), "e2e comment").await
    }
    ...
    async fn validate(
        ...
        let row_ok = row.user_id == setup.session.user_id
            && row.media_id == MediaId(setup.media_id)
            && row.content == "e2e comment";
        let media_ok = media_entity::Entity::find_by_id(MediaId(setup.media_id))
            .one(&ctx.db)
            .await
            .unwrap()
...
impl Scenario for PublishCommentEmptyScenario {
    ...
    async fn run(
        ...
        ctx.client
            .request(
                reqwest::Method::POST,
                &format!("/media/comment/{}", setup.media_id),
            )
    ...
    async fn run(
        ...
        ctx.client
            .request(
                ...
                &format!("/media/comment/{}/{}", setup.media_id, setup.comment_id),
            )
            .header("Authorization", &setup.session.auth_header())
            .send_checked()
            .await?
