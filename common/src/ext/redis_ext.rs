...
impl RedisExt for Pool {
    ...
    async fn get_conn(&self) -> Result<Connection, AppError> {
        self.get()
            .await
            .trace_to_internal_err("redis_get_conn_err", "redis 连接获取错误")
    }
    ...
    async fn get_as<T: FromRedisValue + Send + Sync>(
        ...
        let result: Option<T> = conn
            .get(key.as_ref())
            .await
            .trace_to_internal_err("redis_query_err", "redis 获取错误")?;
        Ok(result)
    }
    ...
    async fn set_ex<T: ToSingleRedisArg + Send + Sync>(
        ...
        conn.set_ex(key.as_ref(), value, ttl)
            .await
            .trace_to_internal_err("redis_set_err", "Redis 写入失败")
    }
    ...
    async fn delete(&self, key: impl AsRef<str> + Send + Sync) -> Result<(), AppError> {
        ...
        let _: () = conn
            .del(key.as_ref())
            .await
            .trace_to_internal_err("redis_del_err", "redis删除key错误")?;
        Ok(())
    }
}
        ...
        M: Fn(&V) -> K + Send + Sync,
    {
        ...
        let cached_jsons: Vec<Option<String>> = {
            let mut conn = self
                .get()
                .await
                .trace_to_internal_err("get_redis_conn_err", "Redis连接获取失败")?;
        ...
        if !miss_indices.is_empty() {
            ...
            let mut conn = self
                .get()
                .await
                .trace_to_internal_err("get_redis_conn_err", "Redis连接获取失败(回写)")?;
