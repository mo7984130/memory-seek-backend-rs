// Media 模块监控 dashboard
// 渲染:sh generate.sh(输出到 ../media.json)
...
local faceExtras(y) = [
  ...
  faceStat(5101, '运行状态', 'media:face_compute:running', 0, 4, y, {
  ...
  faceStat(5102, '计算模式', 'media:face_compute:mode', 4, 4, y, {
    color: { mode: 'thresholds' },
    mappings: [
      {
        options: {
  ...
  faceStat(5103, '当前批次', 'media:face_compute:batch', 8, 4, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
  }, baseStatOptions),
  // 累计 counter
  faceStat(5104, '累计媒体', 'media:face_compute:total_medias', 12, 3, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
    unit: 'short',
  }, baseStatOptions { graphMode: 'area' }),
  faceStat(5105, '累计人脸', 'media:face_compute:total_faces', 15, 3, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
    unit: 'short',
  }, baseStatOptions { graphMode: 'area' }),
  faceStat(5107, '无人脸数', 'media:face_compute:total_no_face', 18, 3, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
    unit: 'short',
  ...
  faceTrend(5108, '处理速率', 0, y + 4, 'smooth', [
    { expr: 'rate(media:face_compute:medias_processed[1m]) * 60', legendFormat: '处理媒体/分钟', refId: 'A' },
    { expr: 'rate(media:face_compute:faces_detected[1m]) * 60', legendFormat: '检出人脸/分钟', refId: 'B' },
    { expr: 'rate(media:face_compute:no_face_medias[1m]) * 60', legendFormat: '无人脸/分钟', refId: 'C' },
  ]),
  faceTrend(5109, '累计趋势', 12, y + 4, 'stepAfter', [
    { expr: 'media:face_compute:total_medias', legendFormat: '累计媒体', refId: 'A' },
    { expr: 'media:face_compute:total_faces', legendFormat: '累计人脸', refId: 'B' },
    { expr: 'media:face_compute:total_no_face', legendFormat: '累计无人脸', refId: 'C' },
  ]),
];
...
local httpPanels = [
  ...
  p.httpQpsPanel('media', 2002, 0, httpPanelY),
  p.httpErrorRatePanel('media', 2003, 8, httpPanelY),
  p.httpLatencyPanel('media', 2004, 16, httpPanelY),
];
...
local opBlocks = std.foldl(
  function(acc, op)
    ...
    local durationPanel =
      p.durationPanel(ops.media.crate, op.name, op.func, idBase + 2, 0, y0 + 1)
      + (if hasSteps then {} else { gridPos: { h: 8, w: 12, x: 0, y: y0 + 1 } });

    local substepsPanels =
      if hasSteps then [p.substepsPanel(ops.media.crate, op.name, op.func, op.steps, idBase + 3, 6, y0 + 1)]
      else [];
...

    {
      ...
      panels:
        ...
        + substepsPanels
        + [p.qpsSuccessPanel(ops.media.crate, op.name, op.func, idBase + 4, 12, y0 + 1)]
        + extraPanels,
    },
  ops.media.ops,
...
local dashboard =
  g.dashboard.new('Media 模块监控')
  + g.dashboard.withUid('media-module-monitoring')
  + g.dashboard.withTags(['media', 'api', 'monitoring'])
