...
pub const MINIMAL_JPEG: &[u8] = &[
    ...
    0xDE, 0x00, 0x00, 0x00, 0x0C, 0x49, 0x44, 0x41, 0x54, 0x08, 0xD7, 0x63, 0xF8, 0xFF, 0x7F, 0x00,
    0x05, 0xFE, 0x02, 0xFE, 0xDC, 0x44, 0x48, 0x30, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44,
    0xAE, 0x42, 0x60, 0x82,
];
...
/// Create a collection and return the collection result JSON.
///
/// Panics on failure.
pub async fn create_collection(
    ...
    let body = match description {
        Some(desc) => serde_json::json!({ "name": name, "description": desc }),
        None => serde_json::json!({ "name": name }),
    };

    let req = auth::auth_request("POST", "/photo/collections", user, body);
    let res = app.clone().oneshot(req).await.unwrap();
    assert_eq!(res.status(), StatusCode::OK, "创建相册失败");
...
pub async fn publish_comment(
    ...
    let res = app.clone().oneshot(req).await.unwrap();
    assert_eq!(res.status(), StatusCode::OK, "发布评论失败");
