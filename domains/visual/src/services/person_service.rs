...
impl PersonService {
    ...
    pub async fn full_scan(state: Arc<VisualState>, admin: AdminId) -> Result<()> {
    ...
    /// 执行全量扫描, 重建人物聚类和人脸归属.
    pub async fn inner_full_scan(state: Arc<VisualState>, admin: AdminId) -> Result<()> {
        let user_id = admin.into_inner();
        info!(user_id = %user_id, "管理员触发人物全量聚类");

        info!("加载照片数据");
        let (faces, visual_file_ids) = PersonRepo::load_faces_with_visual_files(&state).await?;
            ...
            for cluster in cluster_result.0 {
                ...
                let person = NewPerson {
                    ...
                    cover: PersonCover {
                        ...
                        visual_id: cover_face.visual_id,
                        face_score: cover_face.score,
                        file_id: visual_file_ids
                            .get(&cover_face.visual_id)
                            .cloned()
                            .ok_or_error(
                                "person_cover_visual_not_found",
                                "封面人脸所属照片不存在",
                                AppError::InternalServerError,
                            )?,
...
impl PersonService {
    ...
    pub async fn merge_person(
        ...
        PersonView::from_record(
            person,
            admin.into_inner(),
            VisualRepo::get_visual_dimensions(state, file_id).await?,
        )
        .to_ok()
    }
}
...
impl PersonService {
    ...
    /// 获取人物的照片列表(游标分页, 参照 `CollectionVisualService::get_visuals`)
    #[common_macros::metered]
    #[tracing::instrument(
        skip_all,
    ...
    /// 查询人物关联的照片并生成照片视图.
    pub async fn get_person_visuals(
        state: &VisualState,
        user_id: UserId,
        person_id: PersonId,
        req: PersonVisualCursorParam,
    ) -> Result<CursorPage<VisualView, TimeIdCursor<VisualId>>> {
        let page = FaceRepo::query_person_visual_ids(state, person_id, &req)
