// Visual 模块监控 dashboard
// 渲染:sh generate.sh(输出到 ../visual.json)
...
local faceExtras(y) = [
  ...
  faceStat(5101, '运行状态', 'visual:face_compute:running', 0, 4, y, {
  ...
  faceStat(5102, '计算模式', 'visual:face_compute:mode', 4, 4, y, {
    color: { mode: 'thresholds' },
    mappings: [
      {
        options: {
  ...
  faceStat(5103, '当前批次', 'visual:face_compute:batch', 8, 4, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
  }, baseStatOptions),
  // 累计 counter
  faceStat(5104, '累计影像', 'visual:face_compute:total_visuals', 12, 3, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
    unit: 'short',
  }, baseStatOptions { graphMode: 'area' }),
  faceStat(5105, '累计人脸', 'visual:face_compute:total_faces', 15, 3, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
    unit: 'short',
  }, baseStatOptions { graphMode: 'area' }),
  faceStat(5107, '无人脸数', 'visual:face_compute:total_no_face', 18, 3, y, {
    color: { mode: 'thresholds' },
    mappings: [],
    thresholds: { mode: 'absolute', steps: [{ color: 'green', value: null }] },
    unit: 'short',
  ...
  faceTrend(5108, '处理速率', 0, y + 4, 'smooth', [
    { expr: 'rate(visual:face_compute:visuals_processed[1m]) * 60', legendFormat: '处理影像/分钟', refId: 'A' },
    { expr: 'rate(visual:face_compute:faces_detected[1m]) * 60', legendFormat: '检出人脸/分钟', refId: 'B' },
    { expr: 'rate(visual:face_compute:no_face_visuals[1m]) * 60', legendFormat: '无人脸/分钟', refId: 'C' },
  ]),
  faceTrend(5109, '累计趋势', 12, y + 4, 'stepAfter', [
    { expr: 'visual:face_compute:total_visuals', legendFormat: '累计影像', refId: 'A' },
    { expr: 'visual:face_compute:total_faces', legendFormat: '累计人脸', refId: 'B' },
    { expr: 'visual:face_compute:total_no_face', legendFormat: '累计无人脸', refId: 'C' },
  ]),
];
...
local httpPanels = [
  ...
  p.httpQpsPanel('visual', 2002, 0, httpPanelY),
  p.httpErrorRatePanel('visual', 2003, 8, httpPanelY),
  p.httpLatencyPanel('visual', 2004, 16, httpPanelY),
];
...
local opBlocks = std.foldl(
  function(acc, op)
    ...
    local durationPanel =
      p.durationPanel(ops.visual.crate, op.name, op.func, idBase + 2, 0, y0 + 1)
      + (if hasSteps then {} else { gridPos: { h: 8, w: 12, x: 0, y: y0 + 1 } });

    local substepsPanels =
      if hasSteps then [p.substepsPanel(ops.visual.crate, op.name, op.func, op.steps, idBase + 3, 6, y0 + 1)]
      else [];
...

    {
      ...
      panels:
        ...
        + substepsPanels
        + [p.qpsSuccessPanel(ops.visual.crate, op.name, op.func, idBase + 4, 12, y0 + 1)]
        + extraPanels,
    },
  ops.visual.ops,
...
local dashboard =
  g.dashboard.new('Visual 模块监控')
  + g.dashboard.withUid('visual-module-monitoring')
  + g.dashboard.withTags(['visual', 'api', 'monitoring'])
