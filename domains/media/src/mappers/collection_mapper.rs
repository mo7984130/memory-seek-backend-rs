...
use types::media::collection::{self, CollectionId};
use types::media::collection_media;
use types::media::media::MediaId;
use types::{auth::user::UserId, media::collection::*};
...
impl CollectionMapper {
    ...
    pub async fn add_medias_batch(
        ...
        let collection_media_table_name = collection_media::Entity.table_name();
        let collection_table_name = collection::Entity.table_name();

        let cp_collection_id = collection_media::Column::CollectionId.to_string();
        let cp_media_id = collection_media::Column::MediaId.to_string();
        let cp_user_id = collection_media::Column::UserId.to_string();
        let c_id = collection::Column::Id.to_string();
        let c_media_count = collection::Column::MediaCount.to_string();

        let sql = format!(
            r#"
                WITH ins AS (
                    INSERT INTO "{collection_media_table_name}" ("{cp_collection_id}", "{cp_media_id}", "{cp_user_id}")
                    SELECT $1, unnest($2::bigint[]), $3
                    ON CONFLICT ("{cp_collection_id}", "{cp_media_id}") DO NOTHING
                    RETURNING 1
                )
                UPDATE "{collection_table_name}"
                SET "{c_media_count}" = "{c_media_count}" + (SELECT count(*) FROM ins)
                WHERE "{c_id}" = $1
                RETURNING "{c_media_count}"
                "#
        );
        ...
        let stmt = Statement::from_sql_and_values(
            DbBackend::Postgres,
            &sql,
            [collection_id.into(), media_ids.into(), user_id.into()],
        );
        ...
        let new_count: i64 = result.try_get("", &c_media_count)?;

        u64::try_from(new_count).map_err(|error| {
            ContextualError::error(
                "collection_media_count_negative",
                "media_count 异常为负值",
                error,
...
impl CollectionMapper {
    /// 更新收藏夹信息
    pub async fn update_cover_media(
        db: &impl ConnectionTrait,
        collection_id: CollectionId,
        ...
        Entity::update_many()
            .col_expr(Column::CoverMediaId, Expr::value(cover_media_id))
    ...
    pub async fn update_media_count_delta_batch(
        ...
        let col_media_count = Column::MediaCount.as_str();
        ...
        let stmt = Statement::from_sql_and_values(
            ...
            format!(
                r#"
                    UPDATE {table} c
                    SET {col_media_count} = c.{col_media_count} + (delta.cnt)
    ...
    pub async fn update_media_count_delta(
        ...
        Entity::update_many()
            .col_expr(Column::MediaCount, Expr::col(Column::MediaCount).add(delta))
            .filter(Column::Id.eq(collection_id))
            .exec(db)
            .await?;
        Ok(())
