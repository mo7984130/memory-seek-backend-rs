...
use types::{
    ...
    media::{
        ...
        dto::media::MediaView,
        dto::person::{
            MergePersonParam, PersonCursorParam, PersonMediaCursorParam, PersonSearchParam,
...
use crate::{MediaState, services::person_service::PersonService};
...
impl ControllerRouter for PersonController {
    ...
    fn protected_routes() -> axum::Router<std::sync::Arc<Self::State>> {
        Router::new()
            ...
            .route("/{person_id}/medias", get(Self::get_person_medias))
...
impl PersonController {
    ...
    ) -> Result<R<CursorPage<PersonView, CountIdCursor<PersonId>>>> {
    ...
    pub async fn get_person_medias(
        ...
        Extension(user_id): Extension<UserId>,
        ValidatedPath(person_id): ValidatedPath<PersonId>,
        ValidatedQuery(req): ValidatedQuery<PersonMediaCursorParam>,
    ) -> Result<R<CursorPage<MediaView, TimeIdCursor<MediaId>>>> {
        PersonService::get_person_medias(&state, user_id, person_id, req)
            .await
            .to_r_ok()
    }
}
