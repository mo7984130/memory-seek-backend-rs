...
use types::{
    ...
    media::{
        FaceView,
        dto::face::{FaceDeleteBatchParam, FaceDeleteBatchResult, UnassignedFaceMediaCursorParam},
        dto::media::MediaView,
        face::FaceId,
        media::MediaId,
        person::PersonId,
...
use crate::{MediaState, services::face_service::FaceService};
...
impl ControllerRouter for FaceController {
    ...
    fn protected_routes() -> axum::Router<std::sync::Arc<Self::State>> {
        Router::new()
            ...
            .route("/media/{media_id}", get(Self::get_faces_by_media_id))
            .route("/unassigned-medias", get(Self::get_unassigned_face_medias))
            .route(
...
impl FaceController {
    /// 查询照片中的人脸列表.
    async fn get_faces_by_media_id(
        State(state): State<Arc<MediaState>>,
        ValidatedPath(media_id): ValidatedPath<MediaId>,
    ) -> Result<R<Vec<FaceView>>> {
        FaceService::get_faces_by_media_id(&state, media_id)
            .await
            .to_r_ok()
    }
    ...
    /// 分页查询包含未分配人脸的照片.
    async fn get_unassigned_face_medias(
        State(state): State<Arc<MediaState>>,
        Extension(user_id): Extension<UserId>,
        ValidatedQuery(req): ValidatedQuery<UnassignedFaceMediaCursorParam>,
    ) -> Result<R<CursorPage<MediaView, TimeIdCursor<MediaId>>>> {
        FaceService::get_unassigned_face_medias(&state, user_id, req)
            .await
            .to_r_ok()
    }
}
