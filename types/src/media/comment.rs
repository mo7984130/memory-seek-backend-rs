...
mod entity {
    ...
    #[sea_orm(table_name = "media_comment")]
    pub struct Model {
        /// 主键ID
        #[sea_orm(primary_key)]
        pub id: CommentId,
    ...
    /// 创建索引
    /// MediaId 和 CreatedAt 复合索引, 用于 查询照片的评论
    /// MediaId 和 LikeCount 复合索引, 用于 查询照片的热门评论
    ...
    async fn init_index(db: &DatabaseConnection) -> ContextualResult<()> {
        let stmt = Index::create()
            .name("idx_media_comment_media_id_created_at")
            .table(Entity)
            .col(Column::MediaId)
        ...
        let stmt = Index::create()
            .name("idx_media_comment_media_id_like_count")
            .table(Entity)
            .col(Column::MediaId)
