...
# 抓取服务端 Prometheus metrics(9090)快照, 归档到 results/metrics/。
...
if curl -sf --max-time 10 "http://${REMOTE_HOST}:${METRICS_PORT}/metrics" -o "$out"; then
    lines=$(wc -l < "$out" | tr -d ' ')
    echo "[prometheus] 快照已保存: $out ($lines 行)"
else
    echo "[prometheus] 警告: 无法访问 ${REMOTE_HOST}:${METRICS_PORT}, 请确认 server 已启用 metrics feature"
    exit 1
fi
