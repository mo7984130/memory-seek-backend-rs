...
impl Scenario for DownloadOriginalScenario {
    ...
    async fn setup(ctx: &Self::Ctx, task: &TaskIndex) -> Result<Self::Setup, Self::Error> {
        ...
        let view: MediaView = super::upload(ctx, &session, "e2e.png", bytes.clone())
            .await?
            .data;
        Ok(DownloadSetup {
            bytes,
    ...
    async fn run(
        ...
        let resp = ctx
            ...
            .request(
                reqwest::Method::GET,
                &format!("/media/{}", setup.original_token),
            )
            .send_checked()
            .await?;
...
impl Scenario for DownloadInvalidTokenScenario {
    ...
    async fn run(
        ...
        Ok(ctx
            .client
            .request(reqwest::Method::GET, "/media/not_a_valid_image_token")
            .send()
            .await?
            .status()
            .as_u16())
