...
#[cfg(feature = "orm")]
use crate::media::ImageToken;
use crate::media::media::MediaId;
#[cfg(feature = "orm")]
use crate::media::media::MediaRecord;

crate::out_dto!(MediaView, "media/", rename = "Media"; {
    pub id: MediaId,
    pub user_id: UserId,
    pub name: String,
    pub width: u32,
...
mod orm_tests {
    use super::*;
    use crate::media::ImageTokenType;
    ...
    fn from_record_with_tokens_preserves_fields_and_generates_tokens() {
        ...
        let view = MediaView::from_record_with_tokens(record, viewer).unwrap();

        assert_eq!(view.id, MediaId(1));
        assert_eq!(view.user_id, UserId(2));
        assert_eq!(view.name, "media.jpg");
...
impl Default for MediaCursorParam {
    fn default() -> Self {
        Self {
            cursor: None,
            size: 32,
