...
use types::{
    cursor::TimeIdCursor,
    media::{face::*, person::PersonId, media::MediaId},
};
...
impl FaceMapper {
    ...
    /// 按照片 id 加行锁批量查询人脸
    pub async fn lock_by_media_ids(
        db: &impl ConnectionTrait,
        media_ids: &[MediaId],
    ) -> Result<Vec<FaceRecord>> {
        Entity::find()
    ...
    pub async fn query_face_compute_medias(
        ...
        let condition = if full {
            Condition::all().add(types::media::media::Column::Id.gt(previous_id))
        } else {
            let subquery = Query::select()
                .expr(Expr::val(1))
                .from(types::media::face::Entity)
                .and_where(
                    Expr::col((
                        types::media::face::Entity,
                        types::media::face::Column::MediaId,
                    ))
                    .equals((types::media::media::Entity, types::media::media::Column::Id)),
                )
                .to_owned();
            Condition::all()
                .add(types::media::media::Column::Id.gt(previous_id))
                .add(Expr::exists(subquery).not())
        };
        types::media::media::Entity::find()
            .select_only()
            .column(types::media::media::Column::Id)
            .column(types::media::media::Column::FileId)
            .filter(condition)
            .order_by(types::media::media::Column::Id, sea_orm::Order::Asc)
            .limit(size)
            .into_tuple::<(MediaId, String)>()
            .all(db)
            .await
            .map_err(Into::into)
    }
    ...
    pub async fn query_unassigned_face_media_ids_cursor_page(
        db: &impl ConnectionTrait,
        ...
        let subquery = Query::select()
            ...
            .from(Entity)
            .and_where(
                Expr::col((Entity, Column::MediaId))
                    .equals((types::media::media::Entity, types::media::media::Column::Id)),
            )
            .and_where(Column::PersonId.is_null())
            .to_owned();

        let mut query = types::media::media::Entity::find()
            .filter(Expr::exists(subquery))
            .order_by(types::media::media::Column::CreatedAt, Order::Desc)
            .order_by(types::media::media::Column::Id, Order::Desc)
            .limit(size + 1);

        if let Some(cursor) = cursor {
            query = query.filter(cursor.before(
                types::media::media::Column::CreatedAt,
                types::media::media::Column::Id,
            ));
        }
