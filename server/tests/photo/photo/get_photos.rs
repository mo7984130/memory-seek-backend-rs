...
/// Test cursor pagination with size parameter
#[tokio::test]
async fn test_get_photos_pagination_size() {
    let app = build_test_router().await;
    let mut guard = CleanupGuard::new().await;
    ...
    // Request with size=1
    let req = auth::auth_request("GET", "/photo?size=1", &user, serde_json::json!(null));
    let res = app.oneshot(req).await.unwrap();
    ...
    assert_eq!(json["code"], 200);
    assert!(json["data"]["records"].as_array().unwrap().len() <= 1);
