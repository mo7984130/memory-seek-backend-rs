...
impl CommentLikeService {
    ...
    pub async fn like(state: &VisualState, user_id: UserId, comment_id: CommentId) -> Result<()> {
        // 检查评论是否存在
...
impl CommentLikeService {
    ...
    pub async fn unlike(state: &VisualState, user_id: UserId, comment_id: CommentId) -> Result<()> {
