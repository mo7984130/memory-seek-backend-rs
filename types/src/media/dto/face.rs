use crate::media::face::{FaceId, FaceRecord};
use crate::media::models::FaceIds;
use crate::media::person::PersonId;
use crate::media::media::MediaId;
use validator::Validate;

/// 人脸边界框（归一化坐标）——统一复用 `crate::media::image_token::FaceBBox`
use crate::media::FaceBBox;

crate::out_dto!(FaceView, "media/", rename = "Face"; {
    pub id: FaceId,
    pub bbox: FaceBBox,
    /// 归属人物 ID（未分配时为 null）
...
/// 返回未分配人脸照片分页的默认页大小.
fn unassigned_face_media_cursor_page_default_size() -> u64 {
    32
}

crate::in_dto!(UnassignedFaceMediaCursorParam, "media/", docs = "未分配人脸照片游标参数(cursor 为 TimeIdCursor<MediaId> 的 Base64 编码)"; {
    #[cfg_attr(feature = "ts", ts(type = "string | null"))]
    pub cursor: Option<crate::cursor::TimeIdCursor<MediaId>>,
    #[validate(range(min = 1, max = 1024, message = "分页大小在 1 到 1024 之间"))]
    #[serde(default = "unassigned_face_media_cursor_page_default_size")]
    #[cfg_attr(feature = "ts", ts(type = "number"))]
    #[cfg_attr(feature = "ts", ts(optional = nullable))]
    pub size: u64,
});

crate::in_dto!(FaceDeleteBatchParam, "media/", docs = "批量删除人脸参数（仅限未归属人脸）"; {
    #[validate(nested)]
    pub face_ids: FaceIds,
});

crate::out_dto!(FaceDeleteBatchResult, "media/", Default; {
    /// 实际删除的人脸数量
    #[cfg_attr(feature = "ts", ts(type = "number"))]
