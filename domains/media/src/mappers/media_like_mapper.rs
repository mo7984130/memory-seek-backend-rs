...
use types::media::media_like::*;
use types::{auth::user::UserId, media::media::MediaId};

pub struct MediaLikeMapper;
...
impl MediaLikeMapper {
    ...
    pub async fn insert(
        ...
        let rows_affected = Entity::insert(active_model)
            .on_conflict(
                sea_orm::sea_query::OnConflict::columns([Column::MediaId, Column::UserId])
                    .do_nothing()
                    .to_owned(),
            )
...
impl MediaLikeMapper {
    ...
    pub async fn query_user_liked_media_ids(
        ...
        let mut query = Entity::find()
            ...
            .order_by_desc(Column::MediaId);

        if let Some(c) = cursor {
            query = query.filter(c.before(Column::CreatedAt, Column::MediaId));
        }

        let records = query
            .limit(size + 1)
            .into_tuple::<(MediaId, DateTime)>()
            .all(db)
            .await?;
