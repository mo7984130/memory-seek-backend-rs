...
impl FaceController {
    /// 查询照片中的人脸列表.
    async fn get_faces_by_visual_id(
        State(state): State<Arc<VisualState>>,
        ValidatedPath(visual_id): ValidatedPath<VisualId>,
    ) -> Result<R<Vec<FaceView>>> {
        FaceService::get_faces_by_visual_id(&state, visual_id)
            .await
            .to_r_ok()
    }
    ...
    /// 分页查询包含未分配人脸的照片.
    async fn get_unassigned_face_visuals(
        State(state): State<Arc<VisualState>>,
        Extension(user_id): Extension<UserId>,
        ValidatedQuery(req): ValidatedQuery<UnassignedFaceVisualCursorParam>,
    ) -> Result<R<CursorPage<VisualView, TimeIdCursor<VisualId>>>> {
        FaceService::get_unassigned_face_visuals(&state, user_id, req)
            .await
            .to_r_ok()
    }
}
