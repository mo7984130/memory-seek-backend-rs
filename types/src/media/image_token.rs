...
//! 由 `common` 下沉至 `types`，供 media / user / auth 各域复用，并可随 DTO 一起导出 TS。
...
#[cfg_attr(feature = "ts", ts(export, export_to = "media/", rename = "FaceBBox"))]
pub struct FaceBBox {
    pub x1: f32,
    pub y1: f32,
    pub x2: f32,
...
#[cfg_attr(
    feature = "ts",
    ts(export, export_to = "media/", rename = "ImageDimensions")
)]
